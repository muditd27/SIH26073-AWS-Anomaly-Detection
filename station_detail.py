...
def create_trend_chart(
    ...
    if not df.empty and actual_col in df.columns and expected_col in df.columns:
        ...
        padding = max(diff * 0.15, 1.0)
        y_range = [min_val - padding, max_val + padding]

        # Expected line (dashed blue)
        fig.add_trace(
            go.Scatter(
                x=x_vals,
                y=expected_vals,
...
def render_station_detail(station: dict) -> None:
    ...
    # 1. Top Detail Header (matches Image 2 without any raw code block glitch)
    top_html = f"""
    <div class="detail-head">
      ...
      <div class="detail-meta">
        ...
        <div class="meta-card meta-card--ring">
          {health_ring(station.get('sensorHealth', 95), tone, 54)}
    ...
    <div class="kpi-row">
      ...
      <article class="kpi kpi--reason {alert}">
        ...
        <strong class="{anom_color}" style="font-size:15px; margin-top:4px; display:block;">{anomaly_status.upper()}</strong>
        <p style="margin-top:6px; font-size:11px; line-height:1.4;"><b>SHAP Reason:</b> {reason}</p>
        <p style="margin-top:4px; font-size:11px; color:#475569;"><b>Action:</b> {action}</p>
      </article>
    </div>
    """
    ...
    with left:
        ...
        if not trend_df.empty:
            ...
            with head_col1:
                render_html("<div style='font-size:13px; font-weight:700; color:#1e293b; padding-top:6px;'>🌡 Temperature (°C)</div>")
            with head_col2:
                if st.button("⛶ Expand", key="btn_fs_temp", use_container_width=True):
            ...
            head_col1, head_col2 = st.columns([0.78, 0.22])
            with head_col1:
                render_html("<div style='font-size:13px; font-weight:700; color:#1e293b; padding-top:6px;'>◎ Pressure (hPa)</div>")
            with head_col2:
                if st.button("⛶ Expand", key="btn_fs_press", use_container_width=True):
            ...
            # 3. Relative Humidity Graph
            head_col1, head_col2 = st.columns([0.78, 0.22])
            with head_col1:
                render_html("<div style='font-size:13px; font-weight:700; color:#1e293b; padding-top:6px;'>💧 Relative Humidity (%)</div>")
            with head_col2:
                if st.button("⛶ Expand", key="btn_fs_hum", use_container_width=True):
        ...
        with tab_anomalies:
            if anomalies_list:
                ...
                for a in anomalies_list:
                    ...
                    rows.append(
                        f"<tr>"
                        f"<td><b>#{a.get('id', '—')}</b></td>"
                        f"<td>{a.get('timestamp', '—')}</td>"
                        f"<td><span class='badge {badge_class}'>{sev}</span></td>"
                        f"<td><code>{a.get('type', 'ANOMALY')}</code></td>"
                        f"<td style='font-size:11px; color:#334155; line-height:1.3;'>{a.get('message', 'Deviation detected')}<br><span style='color:#64748b;'><b>Action:</b> {a.get('action', 'Inspect')}</span></td>"
                        f"</tr>"
                    )
                ...
                <div class="panel">
                  ...
                  <table class="raw">
                    <thead>
                      <tr>
                        <th>ID</th><th>Time</th><th>Severity</th><th>Type</th><th>SHAP Root Cause / Action</th>
                      </tr>
                    </thead>
