...
from data import anomaly_class
...
def station_card(station: dict) -> str:
    ...
    <a class="station-card" href="?station={station['id']}" target="_self">
      ...
      <div class="station-card-bottom">
        ...
        <div class="anomaly-cell {anom}">{icons[anom]}<div><strong>{station['anomalyStatus']}</strong><span>{anom_sub}</span></div></div>
...
def render_stations_page(stations: list[dict]) -> str:
    if not stations:
        cards_html = """
        <div class="panel" style="padding: 24px; text-align: center; grid-column: 1 / -1;">
          <h3>⚠️ Unable to connect to FastAPI Backend</h3>
          <p style="color: #64748b;">Please verify that FastAPI is running at <code>http://127.0.0.1:8000</code>.</p>
        </div>
        """
    else:
    ...
    <section class="hero">
      ...
      <div class="hero-visual">
        <div class="spark-wrap">
          <div class="hero-alert">⚠ Anomaly Detection Active</div>
          {sparkline()}
        </div>
        <ul class="pipeline">
          <li>✓ Detect</li>
